...
class Moderation(commands.Cog):
    ...
    async def mute(self, interaction: discord.Interaction, membre: discord.Member, duree: int = 10, raison: str = "Aucune raison"):
        if not self.check(interaction, "moderate_members"):
            return await interaction.response.send_message("❌ Permission refusée.", ephemeral=True)
        until = discord.utils.utcnow() + timedelta(minutes=duree)
        await membre.timeout(until, reason=raison)
        await interaction.response.send_message(embed=self.embed("🔇 Mute", f"**{membre}** mute **{duree} min**.\n**Raison :** {raison}", discord.Color.orange()))
    ...
    async def warn(self, interaction: discord.Interaction, membre: discord.Member, raison: str = "Aucune raison"):
        if not self.check(interaction, "manage_messages"):
            return await interaction.response.send_message("❌ Permission refusée.", ephemeral=True)
        warns = load_warns()
        uid = str(membre.id)
        if uid not in warns:
        ...
        save_warns(warns)
        count = len(warns[uid])
        await interaction.response.send_message(embed=self.embed("⚠️ Warn", f"**{membre}** averti.\n**Raison :** {raison}\n**Total :** {count}", discord.Color.yellow()))
    ...
    async def clearwarn(self, interaction: discord.Interaction, membre: discord.Member):
        if not self.check(interaction, "administrator"):
            return await interaction.response.send_message("❌ Permission refusée.", ephemeral=True)
        warns = load_warns()
        warns[str(membre.id)] = []
        save_warns(warns)
        await interaction.response.send_message(embed=self.embed("🧹 Warns effacés", f"Warns de **{membre}** supprimés.", discord.Color.green()))
    ...
    async def lock(self, interaction: discord.Interaction):
        if not self.check(interaction, "manage_channels"):
            return await interaction.response.send_message("❌ Permission refusée.", ephemeral=True)
        overwrite = interaction.channel.overwrites_for(interaction.guild.default_role)
        overwrite.send_messages = False
        await interaction.channel.set_permissions(interaction.guild.default_role, overwrite=overwrite)
        await interaction.response.send_message(embed=self.embed("🔒 Verrouillé", f"{interaction.channel.mention} verrouillé."))
    ...
    async def unlock(self, interaction: discord.Interaction):
        ...
        await interaction.channel.set_permissions(interaction.guild.default_role, overwrite=overwrite)
        await interaction.response.send_message(embed=self.embed("🔓 Déverrouillé", f"{interaction.channel.mention} ouvert.", discord.Color.green()))
    ...
    async def slowmode(self, interaction: discord.Interaction, secondes: int):
        ...
        msg = f"Mode lent : **{secondes}s**" if secondes > 0 else "Mode lent désactivé."
    ...
    async def unban(self, interaction: discord.Interaction, user_id: str):
        ...
        try:
            ...
            await interaction.response.send_message(embed=self.embed("✅ Unban", f"**{user}** débanni.", discord.Color.green()))
        except Exception:
